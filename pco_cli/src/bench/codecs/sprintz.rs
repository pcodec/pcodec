...
impl CodecInternal for SprintzConfig {
  ...
  fn compress<T: PcoNumber>(&self, nums: &[T]) -> Vec<u8> {
    ...
    let clen = unsafe {
      ...
      match (size_of::<T>(), &self.filter) {
        (1, Filter::Delta) => {
          sprintz_sys::sprintz_c_compress_delta_8b(nums.as_ptr() as _, len as u32, dest as _, 1)
        }
        (1, Filter::Xff) => {
          sprintz_sys::sprintz_c_compress_xff_8b(nums.as_ptr() as _, len as u32, dest as _, 1)
        }
        (2, Filter::Delta) => {
          sprintz_sys::sprintz_c_compress_delta_16b(nums.as_ptr() as _, len as u32, dest as _, 1)
        }
        (2, Filter::Xff) => {
          sprintz_sys::sprintz_c_compress_xff_16b(nums.as_ptr() as _, len as u32, dest as _, 1)
        }
  ...
  fn decompress<T: PcoNumber>(&self, compressed: &[u8]) -> Vec<T> {
    ...
    let dlen = unsafe {
      ...
      match (size_of::<T>(), &self.filter) {
        (1, Filter::Delta) => {
          sprintz_sys::sprintz_c_decompress_delta_8b(compressed.as_ptr() as _, dest as _)
        }
        (1, Filter::Xff) => {
          sprintz_sys::sprintz_c_decompress_xff_8b(compressed.as_ptr() as _, dest as _)
        }
        (2, Filter::Delta) => {
          sprintz_sys::sprintz_c_decompress_delta_16b(compressed.as_ptr() as _, dest as _)
        }
        (2, Filter::Xff) => {
          sprintz_sys::sprintz_c_decompress_xff_16b(compressed.as_ptr() as _, dest as _)
        }
        _ => panic!("sprintz only supports 1- and 2-byte types"),
      }
    };
