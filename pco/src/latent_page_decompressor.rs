...
impl<L: Latent> LatentPageDecompressor<L> {
  ...
  unsafe fn decompress_ans_symbols(&mut self, reader: &mut BitReader, batch_n: usize) {
    ...
    for i in 0..batch_n {
      ...
      self
        .state
        .set_scratch(i, offset_bit_idx, node.offset_bits(), info);
...
impl DynLatentPageDecompressor {
  pub fn create<L: Latent>(
    ...
    let offset_bits = bins.iter().map(|bin| bin.offset_bits).collect::<Vec<_>>();
    let decoder = ans::Decoder::new(&ans_spec, &offset_bits);
