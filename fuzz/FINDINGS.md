...
What the libFuzzer targets in `fuzz/fuzz_targets/` turned up. The fixes live
elsewhere: findings 1-5 in #380 (merged, released in 1.0.3), findings 6-8 in
the C API PR. This file is the record of how each one is reached, so the
targets can be re-pointed at them later; line numbers and quoted code refer to
the tree as of 1.0.2, before the fixes.
...
All five fixed by #380, merged and shipped in 1.0.3; the regression tests for
them are in `pco/src/tests/corruption.rs`. Fuzzing after those fixes, with no
crash found:
...
Both targets link `cpcodec` as a Rust dependency, which needs `rlib` in its
crate-type -- that one-line change is part of the C API PR, so `cargo fuzz
build c_api_roundtrip` only works with that branch in the tree.
